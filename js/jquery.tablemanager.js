...
(function($){
  ...
  var methods = {
    init : function(t, args){
      ...
      var isMobile = function(){
          ...
          is_iPod = navigator.platform.indexOf("iPod") != -1;
          is_iPhone = navigator.platform.indexOf("iPhone") != -1;
          is_Android = navigator.userAgent.match(/Android/i) != null;
          is_BlackBerry = navigator.userAgent.match(/BlackBerry/i) != null;
      ...
      if(mobile_ui_mode || (enable_column_collapse_on_desktop && args.mincols == undefined || args.mincols <= table.data('width'))){
          ...
          table.find("tbody th." + cssclasses.collapsible).click(function(){
              methods.toggle_row($(this));
          });
      }
      else{
    ...
    toggle_row : function(cl){
      ...
      var toggle = collapsed(cell) ? function(cl) { 
          cl.removeClass("row-nonvisible");
          if(!cl.hasClass("col-nonvisible")){
              cl.children("p,span").show();
          }
      }  : function(cl) { 
          cl.addClass("row-nonvisible");
          cl.children("p,span").hide();
      };
  ...
  function get_next_collapsible_physrow(table, col, row) {
    ...
    while (y < h) {
      ...
      else if(co[arrpos(col, y, w)].attr('class').indexOf(cssclasses.inherited) >= 0){
        y++;
      }
      else{
        return y - 1;
      ...
      for (var j = 0; j < i; j++) {
        ...
        if(mobile_ui_mode){
            li.click(function(evt){
                evt.stopPropagation();
                methods.toggle_col($(this).find('img').data('applied_cell'));
            });
        }else{
            img.click(function(){
