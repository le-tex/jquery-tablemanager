...
(function($){
  ...
  function method_dispatcher(method, args){
    if (pub_methods[method]) {
      return pub_methods[method].apply(this, args);
    }
    else if(typeof method === 'object' || ! method ){
      return pub_methods.init.call(this, method, args);
    }
    else {
  ...
  var pub_methods = {
    ...
    toggle_row : function(cl){
      ...
      var toggle = collapsed(cell) ? function(cl) { cl.children("*").show(); cl.each(function(){$(this).removeClass("row-nonvisible")});} : function(cl) { cl.children("*").hide(); cl.each(function(){$(this).addClass("row-nonvisible")});};
      ...
      dependent_cells.each( function() { 
	if($(this).hasClass("row-nonvisible")){
            $(this).removeClass("row-nonvisible");
        }
        if(!$(this).hasClass("col-nonvisible")){
          toggle($(this));
        }
  ...
  function listify_head_row(table, xstart, xend, ystart) {
    ...
    for (var y = ystart; ! found && y < headheight; y++) {
      ...
      for (var j = 0; j < i; j++) {
        ...
        img.click(function(){
          pub_methods.toggle_col($(this).data('applied_cell'));
        });
